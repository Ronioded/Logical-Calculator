...
/**
 * @author Roni Oded.
 * Class Or, caractories the logic expression Or.
...
public class Or extends BinaryExpression {
    ...
    public Expression simplify() {
        ...
        Expression newExp = new Or(exp1, exp2);

        /* if exp1 and exp2 is equal,newExp is now the expression. */
        if (exp1.isEqual(exp2)) {
            return exp1;
        }

        /* try to evaluate exp1, if it's true,newExp is now true. if it throws exception, there is no way to evaluate
        ...
        /* try to evaluate exp2, if it's true,newExp is now true. if it throws exception, there is no way to evaluate
         it so there is no way to simplify in this case, catch the exception and moving to the next condition. */
        try {
            if (exp2.evaluate()) {
                newExp = new Val(true);
